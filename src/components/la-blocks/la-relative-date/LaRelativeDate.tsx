...
export default function LaRelativeDate({
  ...
  return (
    <button
      ...
      className={cn(
        "shrink-0 text-xs text-slate-400 transition-colors hover:text-slate-600 cursor-pointer",
        className,
      )}
      aria-label="Toggle between relative and exact date"
      title={showExact ? "Show relative time" : "Show exact date"}
    >
      {displayLabel}
    </button>
  );
