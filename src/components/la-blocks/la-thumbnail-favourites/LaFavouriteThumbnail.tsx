    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-1 justify-center">
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex min-w-0 flex-1 items-center gap-1.5 flex-wrap">
            <p className="text-sm font-bold leading-tight text-slate-800 shrink-0">
              {priceLabel}
              {priceSuffix && (
                <span className="ml-1 text-xs font-medium text-slate-500">{priceSuffix}</span>
              )}
            </p>
            {status !== "active" && (
              <span
                className={cn(
                  "shrink-0 rounded px-2 py-1 text-[10px] font-bold uppercase leading-none tracking-wide",
        ...
        <div className="flex items-center gap-1 overflow-hidden">
          <p className="shrink-0 text-xs font-semibold uppercase tracking-wide text-slate-400">{detailsLabel}</p>
          <span className="shrink-0 text-xs text-slate-300 mx-0.5">·</span>
          <Outline_MapPin_24by24 className="h-3 w-3 shrink-0 text-slate-400" />
          <p className="min-w-0 flex-1 truncate text-xs text-slate-400">{locationLabel}</p>
