        ...
        {list.map((c) => {
          ...
          return (
            <button
              ...
              className={`w-full px-4 py-2.5 text-left flex items-center gap-3 hover:bg-slate-200 ${
                isSelected ? "bg-blue-50" : ""
              }`}
            >
              {/* ✅ Emoji flag */}
              <span className="flex-none text-lg">{c.flag}</span>
      ...
      <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
        <DialogContent className="w-full max-w-sm p-0">
          <DialogHeader>
            ...
            {/* ✅ accessibility fix */}
            <DialogDescription>
              Choose your country for phone number
            </DialogDescription>
          </DialogHeader>
          {content}
