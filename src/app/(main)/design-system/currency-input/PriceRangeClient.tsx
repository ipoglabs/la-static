...
function LaAmountRange({
  ...
  const [minInput, setMinInput] = useState(minValue === 0 ? "" : String(minValue));
  const [maxInput, setMaxInput] = useState(maxValue === ABSOLUTE_MAX ? "" : String(maxValue));
  ...
  const handleSelect = (val: number | null) => {
    ...
    if (drawerType === "min") {
      ...
      setMinInput(resolved === 0 ? "" : String(resolved));
      ...
      setMaxInput(resolved === ABSOLUTE_MAX ? "" : String(resolved));
  ...
  const handleInputApply = () => {
    const raw = parseInt(inputText.replace(/,/g, ""), 10);
    if (isNaN(raw) || raw < 0) return;
    handleSelect(raw === 0 && drawerType === "min" ? 0 : raw);
  };

  const inputNumeric = parseInt(inputText.replace(/,/g, ""), 10);
  const inputInvalid =
    inputText !== "" &&
    (isNaN(inputNumeric) ||
      inputNumeric < 0 ||
      (drawerType === "min" && maxValue !== ABSOLUTE_MAX && inputNumeric >= maxValue) ||
      (drawerType === "max" && minValue > 0 && inputNumeric <= minValue));
  ...
  const inputBox = (type: DrawerType) => {
    ...
    const setVal = isMin ? setMinInput : setMaxInput;
      ...
      <div style={{
        ...
        <input
          ...
          onChange={(e) => {
            ...
            setVal(raw);
            const num = raw === "" ? (isMin ? 0 : ABSOLUTE_MAX) : parseInt(raw, 10);
            if (isMin) {
              if (maxValue !== ABSOLUTE_MAX && num >= maxValue) return;
              onMinChange?.(num);
            } else {
              if (num !== ABSOLUTE_MAX && num <= minValue) return;
              onMaxChange?.(num);
            }
          }}
          style={{
  ...
  return (
    <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ fontSize: 12, color: "#888", textAlign: "center" }}>
        {minValue === 0 ? "No Min" : `${sym} ${addCommas(minValue)}`}
        {" — "}
        {maxValue === ABSOLUTE_MAX ? "No Limit" : `${sym} ${addCommas(maxValue)}`}
      </div>
        ...
        <DrawerContent>
          ...
          <div style={{ padding: "12px 20px 8px", borderBottom: "1px solid #f0f0f0" }}>
            <div style={{
              ...
              border: inputInvalid ? "1.5px solid #e53e3e" : "1.5px solid #d8d8d8",
              borderRadius: 12, padding: "0 14px", height: 48, gap: 8, background: "#fafafa",
            }}>
              <span style={{ fontWeight: 600, fontSize: 14, color: "#222", whiteSpace: "nowrap" }}>
                {sym}
              </span>
              <input
                type="text"
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && !inputInvalid && inputText !== "") handleInputApply();
                }}
                style={{
                  flex: 1, border: "none", outline: "none",
                  background: "transparent", fontSize: 16, fontWeight: 500, color: "#222",
                }}
              />
              {inputText !== "" && (
                <button onClick={() => setInputText("")} style={{
                  background: "none", border: "none", cursor: "pointer",
                  padding: 0, color: "#aaa", fontSize: 16, lineHeight: 1,
                  display: "flex", alignItems: "center",
                }}>✕</button>
              )}
            </div>
            ...
            {inputInvalid && (
              <p style={{ fontSize: 12, color: "#e53e3e", margin: "6px 2px 0" }}>
                {drawerType === "min"
                  ? `Must be less than max (${sym} ${addCommas(maxValue)})`
                  : `Must be greater than min (${sym} ${addCommas(minValue)})`}
              </p>
            )}

            {inputText !== "" && !inputInvalid && (
              <button
                onMouseDown={(e) => { e.preventDefault(); handleInputApply(); }}
                style={{
          ...
          <div style={{ overflowY: "auto", flex: 1, paddingBottom: 24 }}>
            {presets.map(({ value: val, label: presetLabel }, i) => {
              const isSelected =
                drawerType === "min"
                  ? val === minValue
                  : maxValue === ABSOLUTE_MAX ? val === null : val === maxValue;

              const isDisabled =
                drawerType === "min"
                  ? maxValue !== ABSOLUTE_MAX && val !== null && val !== 0 && (val as number) >= maxValue
                  : val !== null && minValue > 0 && (val as number) <= minValue;

              return (
                <div key={i}>
                  <div
                    onMouseDown={(e) => { e.preventDefault(); if (!isDisabled) handleSelect(val); }}
  ...
  return (
    <main style={{
      ...
      <div style={{
        ...
        borderRadius: 24, padding: 24, boxShadow: "0 4px 24px rgba(0,0,0,0.06)",
      }}>
